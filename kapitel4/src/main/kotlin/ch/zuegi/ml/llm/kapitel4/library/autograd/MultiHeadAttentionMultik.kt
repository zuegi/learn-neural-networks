...
class MultiHeadAttentionMultik(
    ...
    val wQuery: NDArray<Double, D2> = randomMatrix(embeddingDim, headDim)
    val wKey: NDArray<Double, D2> = randomMatrix(embeddingDim, headDim)
    val wValue: NDArray<Double, D2> = randomMatrix(embeddingDim, headDim)
    val wOutput: NDArray<Double, D2> = randomMatrix(headDim, embeddingDim)
    ...
    fun forward(
        ...
        val concat = processHeads(query, key, value, training)
        return matMul(concat, wOutput)
    }

    private fun processHeads(
        query: NDArray<Double, D2>,
        key: NDArray<Double, D2>,
        value: NDArray<Double, D2>,
        training: Boolean,
    ): NDArray<Double, D2> {
        ...
        for (h in 0 until numHeads) {
            val start = h * dK
            val qH = sliceCols(query, start, dK)
            val kH = sliceCols(key, start, dK)
            val vH = sliceCols(value, start, dK)

            val scores = scaledScores(qH, kH)
            val masked = if (causal) maskCausal(scores) else scores

            var weights = softmaxRows(masked)
            if (training && dropoutProb > 0.0) {
                weights = applyDropout(weights, dropoutProb)
            }

            val headOutput = matMul(weights, vH)

            for (row in 0 until ctx) {
                for (col in 0 until dK) {
                    result[row][start + col] = headOutput[row][col]
                }
            }
        }

        return mk.ndarray(result.map { it.toList() })
    }
    ...
    private fun scaledScores(
        ...
        return mk.ndarray(
            List(ctx) { i ->
                List(ctx) { j ->
                    var sum = 0.0
                    for (d in 0 until dK) sum += q[i][d] * k[j][d]
                    sum / scale
                }
            },
        )
    ...
    private fun softmaxRows(scores: NDArray<Double, D2>): NDArray<Double, D2> {
        ...
        return mk.ndarray(
            List(rows) { i ->
                ...
                val exps = DoubleArray(cols) { c -> exp(scores[i][c] - max) }
                val sum = exps.sum()
                List(cols) { c -> exps[c] / sum }
            },
        )
    }
    ...
    private fun applyDropout(
        ...
        if (p == 1.0) return mk.ndarray(List(rows) { List(cols) { 0.0 } })
    ...
    private fun sliceCols(
        ...
        return mk.ndarray(
            List(rows) { r ->
                List(width) { c -> matrix[r][fromCol + c] }
            },
        )
    }
