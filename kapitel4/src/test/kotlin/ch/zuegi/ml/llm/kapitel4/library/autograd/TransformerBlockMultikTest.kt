...
class TransformerBlockMultikTest {
    @Test
    fun `forward liefert output gleicher Form wie input`() {
        val block = TransformerBlockMultik(embeddingDim = 8, numHeads = 2, dK = 4, seed = 42)
        val input = identityInput(ctx = 3, dim = 8)

        val out = block.forward(input, training = false)
    ...
    fun `gleiches seed und eval modus erzeugen gleiche outputs`() {
        val b1 = TransformerBlockMultik(embeddingDim = 8, numHeads = 2, dK = 4, seed = 7)
        val b2 = TransformerBlockMultik(embeddingDim = 8, numHeads = 2, dK = 4, seed = 7)
        val input = identityInput(ctx = 3, dim = 8)

        val out1 = b1.forward(input, training = false)
        val out2 = b2.forward(input, training = false)
    ...
    fun `dropout im training beeinflusst output`() {
        val noDropout = TransformerBlockMultik(embeddingDim = 8, numHeads = 2, dK = 4, dropoutProb = 0.0, seed = 11)
        val withDropout = TransformerBlockMultik(embeddingDim = 8, numHeads = 2, dK = 4, dropoutProb = 0.5, seed = 11)
        val input = identityInput(ctx = 3, dim = 8)
