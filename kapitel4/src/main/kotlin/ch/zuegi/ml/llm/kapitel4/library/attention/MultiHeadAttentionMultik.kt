package ch.zuegi.ml.llm.kapitel4.library.attention
...
class MultiHeadAttentionMultik(
    ...
    private fun sliceCols(
        ...
        mk.ndarray(
            List(matrix.shape[0]) { i ->
                List(until - from) { d -> matrix[i][from + d] }
            },
        )
    ...
    private fun maskCausal(scores: NDArray<Double, D2>): NDArray<Double, D2> =
        mk.ndarray(
            List(scores.shape[0]) { i ->
                List(scores.shape[1]) { j ->
    ...
    private fun applyDropout(
        ...
        if (p == 1.0) {
            return mk.ndarray(List(weights.shape[0]) { List(weights.shape[1]) { 0.0 } })
        }
        val keepScale = 1.0 / (1.0 - p)
        return mk.ndarray(
            List(weights.shape[0]) { i ->
                List(weights.shape[1]) { j ->
