...
class Tensor(
    ...
    fun softmaxCrossEntropy(target: Int): Tensor {
        ...
        val max = data.max()
        val exps = DoubleArray(size) { exp(data[it] - max) }
        val sumExp = exps.sum()
        val probs = DoubleArray(size) { exps[it] / sumExp }

        val loss = -ln(probs[target])
     ...
     *
     * Mit Wahrscheinlichkeit [probability] wird ein Element auf 0 gesetzt.
     * Behaltene Elemente werden mit `1 / (1 - probability)` skaliert, damit
     * der Erwartungswert im Training unveraendert bleibt.
     *
     * Diese Operation ist fuer Training gedacht. Im Eval-Modus sollte der
     * Aufrufer Dropout gar nicht erst anwenden.
     *
     * Backward:
     * - gedroppte Elemente erhalten keinen Gradienten
     * - behaltene Elemente leiten den Gradienten skaliert weiter
     *
     * @param probability Dropout-Wahrscheinlichkeit in [0.0, 1.0].
     * @param rnd Zufallsquelle fuer reproduzierbare Masken.
     * @return Tensor gleicher Form mit angewandtem Dropout.
     */
    fun dropout(
        probability: Double,
        rnd: Random,
        ...
        require(probability in 0.0..1.0) {
            "probability muss in [0.0, 1.0] liegen"
        }

        if (probability == 0.0) return this

        if (probability == 1.0) {
            val out = Tensor(DoubleArray(size), listOf(this))
            out.backwardStep = {
                // alles gedroppt -> kein Gradient fliesst zurueck
            }
            return out
        }
    ...
    /**
     * Extrahiert aus diesem [rows, cols]-Matrix-Tensor einen zusammenhaengenden
     * Spaltenbereich `[fromCol, fromCol + width)` und liefert ihn als neuen
     * [rows, width]-Tensor (row-major).
     *
     * Backward schreibt den Gradienten an die passenden Ursprungs-Spalten
     * zurueck; alle anderen Spalten erhalten 0.
     *
     * @param rows Anzahl Zeilen der Matrix.
     * @param cols Anzahl Spalten der Matrix.
     * @param fromCol Startspalte inklusive.
     * @param width Anzahl zu extrahierender Spalten.
     * @return Matrix-Tensor [rows, width].
     */
    fun sliceCols(
        rows: Int,
        cols: Int,
        ...
        require(size == rows * cols) { "size $size passt nicht zu rows*cols=${rows * cols}" }
        require(fromCol >= 0) { "fromCol muss >= 0 sein" }
        require(width > 0) { "width muss > 0 sein" }
        require(fromCol + width <= cols) {
            "Spaltenbereich [$fromCol, ${fromCol + width}) ausserhalb 0 until $cols"
        }
    ...
    /**
     * Multipliziert einen Skalar-Tensor der Laenge 1 mit einem Vektor-Tensor
     * elementweise. Gedacht fuer Attention-Gewicht * Value-Zeile.
     *
     * Backward:
     * - dScalar = Summe_i other[i] * dOut[i]
     * - dOther[i] = scalar * dOut[i]
     *
     * @param other Vektor gleicher Ziel-Laenge.
     * @return Vektor-Tensor gleicher Laenge wie [other].
     */
    fun broadcastMul(other: Tensor): Tensor {
    ...
    /**
     * Skalarprodukt zweier Vektoren gleicher Laenge.
     *
     * Ergebnis ist ein Skalar-Tensor der Laenge 1.
     *
     * Backward:
     * - dLeft[i]  += right[i] * dOut
     * - dRight[i] += left[i]  * dOut
     *
     * @param other rechter Vektor gleicher Laenge.
     * @return Skalar-Tensor mit dem Skalarprodukt.
     */
    fun dot(other: Tensor): Tensor {
        ...
        val out = Tensor(doubleArrayOf(sum), listOf(this, other))
        out.backwardStep = {
            val g = out.grad[0]
            for (i in 0 until size) {
    ...
    /**
     * Berechnet das Skalarprodukt eines Vektors [row] mit jeder Zeile dieser
     * [rows, cols]-Matrix. Ergebnis ist ein Vektor der Laenge [rows].
     *
     * @param row Vektor der Laenge [cols].
     * @param rows Anzahl Zeilen dieser Matrix.
     * @param cols Anzahl Spalten dieser Matrix.
     * @return Vektor-Tensor mit einem Skalarprodukt pro Zeile.
     */
    fun dotWithRows(
        row: Tensor,
        rows: Int,
        ...
        val out = Tensor(result, listOf(this, row))
        out.backwardStep = {
